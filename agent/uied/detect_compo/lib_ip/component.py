...
from agent.uied.detect_compo.lib_ip.bbox import Bbox
...
class Component:
  ...
  def compo_get_bbox(self):
    ...
    """
    col_min, row_min = (int(min(self.boundary[0][0][0], self.boundary[1][-1][0])),
                        int(min(self.boundary[2][0][0], self.boundary[3][-1][0])))
    col_max, row_max = (int(max(self.boundary[0][0][0], self.boundary[1][-1][0])),
