...
shared_agent: TestAgent | None = None
shared_device = None
shared_dm = None
...
def initialize():
  ...
  data = request.json
  missing_params = [p for p in
                    ["app_name", "app_package", "app_launch_activity", "scenario_name", "scenario_description"] if
                    not data.get(p)]
  if missing_params:
    ...
    return jsonify(
      {
        ...
        "screenshot_withbbox": url_for('static', filename=f'screenshots/{screenshot_with_bbox_filename}',
                                       _external=True),
        "next_actions": {
